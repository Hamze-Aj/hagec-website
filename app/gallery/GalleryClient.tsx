...
import type { GalleryItem } from "@/lib/get-gallery";
...
export default function GalleryClient({ initialItems }: { initialItems: GalleryItem[] }) {
  ...
  const [lightboxImg, setLightboxImg] = useState<null | GalleryItem>(null);
  ...
  return (
    <>
      ...
      {/* ── Filters ──────────────────────────────────────────── */}
      <section className="bg-white border-b border-[#E2E8F0] py-5 sticky top-[76px] lg:top-20 z-40" aria-label="Gallery filters">
        ...
        <div className="container-custom">
          <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6">
            {filtered.map((img) => (
              <div
                key={img.id}
                className="relative break-inside-avoid rounded-xl overflow-hidden group cursor-pointer shadow-lg hover:shadow-xl transition-all"
